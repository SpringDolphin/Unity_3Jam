...
{    /// <summary>
    ...
    /// 放物線を何秒分計算するか
    /// </summary>
    private float predictionTime = 10.0F;
    ...
    /// </summary>
    private BallManager shootBullet;
    ...
    private  GameManager ballstatusscript;
    ...
    void Start()
    {
        ...
        // 弾の初速度や生成座標を持つコンポーネント
        shootBullet = gameObject.GetComponent<BallManager>();

        //状態を持つコンポーネントの読み込み
        ballstatusscript = gameMaster.GetComponent<GameManager>();
    }
    ...
    void Update()
    {
        ...
        Debug.Log(ballstatusscript.ballstatus);
        if (drawArc&&(ballstatusscript.ballstatus==GameManager.BallStatus.throwed))
        {
            // 放物線を表示
            Debug.Log("放物線");
    ...
    private void ShowPointer(Vector3 position)
    {
        ...
        pointerObject.transform.rotation = Quaternion.Euler(90, 0, 0);
